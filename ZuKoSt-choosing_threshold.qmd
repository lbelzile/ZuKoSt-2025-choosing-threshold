...
Most risk measures of interest are functionals of the parameters.
...
If $Y-u\sim \mathsf{GP}(\sigma_u,\xi)$, then for any $v>u$ such that $\sigma_v=\sigma_u+ \xi(v-u) > 0$, we have 
\begin{align*}
...
\begin{align}
e(v) = \mathrm{E}(X - v \mid X>v) = \frac{\sigma_u - \xi u}{1-\xi} + \frac{\xi}{1-\xi} v, \quad v\geq u.   \label{eq:mrl}
\end{align}
...
## Second-order regular variation {.smaller}
...
- $\sigma_t = r(u_t),$ where $r(t) = \{1-F(t)\}/f(t)$ the reciprocal hazard function,
- $\xi_t = r'(u_t)$ is the penultimate approximation to the shape.

If $u_t$ is twice differentiable and $u_t'$ is eventually positive, then Theorem 2.1 of @deHaan/Resnick:1996 implies $A(t) =\xi_t - \xi,$ the difference between the penultimate and limiting values of the shape parameter.


## Asymptotic normality


Under a second order regular variation assumption with $\lim_{n_u \to \infty} \sqrt{n_u}A(n/n_u) = \lambda\in \mathbb{R}$, subject to which \citep[Theorem 3.2.5]{deHaan.Ferreira:2006}
 \begin{align*}
...
Many heavy-tailed distributions fall within the @Hall.Welsh:1985 class of distributions have $A(t) = \beta t^\rho$ for $\beta \neq 0$ and $\rho < 0.$

The asymptotic mean squared error $\xi^2/n_u + A^2(n/n_u)/(1-\rho)^2$ gives
\begin{align}
...
- Non-nested models, samples change with the threshold.
- Multiple testing problem.
- Potential for automating the selection.
...
Automation proposed by @Langousis:2016 based on weighted regression.
...
## Bootstrap methods based on @Hall:1990
...
Other proposals [@Goegebeur.Beirlant.deWet:2008,@Bladt:2020,@Schneider:2021] relate the bias and MSE of Hill estimator with other estimators for $\xi>0$, and use these to minimize the mean squared error under the assumption $\rho=-1.$
...
- *Ad hoc* methods, e.g., @Thompson:2009, sometimes works well.
...
Slides at [`lbelzile.github.io/ZuKoSt-2025-choosing-threshold`](https://lbelzile.github.io/ZuKoSt-2025-choosing-threshold)
